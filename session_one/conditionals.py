...
val = "test"
if val:
  print("test is the best.")

"""
This is an important point to discuss python scope. If statement would be the first instance where
we run into the need to use scope. Scope in Python is deliniated with indent.
"""
test = 1
if test == 1:
  print("Test is one")
...
#if, elif , else - rock, paper, scisors
import random
import time
seed_val = int(time.time()*1000)
print(seed_val)
random.seed(seed_val)
user = random.choice(["Rock", "Paper", "Scisros"])
pc = random.choice(["Rock", "Paper", "Scisros"])
...
if user == pc:
  ...
  if (user == "Rock" and pc == "Paper") or (user == "Paper" and pc == "Scisors") or (user == "Scisors" and pc=="Rock") :
    print("You loose!")
  else:
    print("You win!")
